...
void Sale_UI_ShowTicket(int schID){
	...
	do {
		...
		for (i = 0, pos = (ticket_node_t*) (paging.curPos);
			...
			printf("%2d %10s %10s %3d %3d %3d %4s\n", pos->data.id,
					ply.name, stu.name, set.row, set.column, pos->data.price,
					(pos->data.status == 0) ?
							("待售") :
							((pos->data.status == 1) ? ("已售") : ("预留")));
			pos = pos->next;
		}
...
void Sale_UI_MgtEntry() {
	...
	do{
		...
		printf("------- 全部记录:%2d ----------------------- 页数 %2d/%2d ----\n",
				...
				Pageing_TotalPages(paging));
		printf("******************************************************************\n");
		printf("[C]演出计划|[S]查找剧目名|[P]上一页|[N]下一页|[R]返回");
		printf("\n==================================================================\n");
		printf("功能选择:");//Your Choice
		scanf("%c", &choice);
		ffflush();
